...
from utils import cuda, grid2gif, print_size
from model import BetaVAE_H, BetaVAE_B, BetaVAE_MNIST
...
class Solver(object):
    def __init__(self, args):
        ...
        if args.dataset.lower() == 'dsprites':
            ...
            self.nc = 3
            self.decoder_dist = 'gaussian'
        elif args.dataset.lower()[:5] == 'mnist':
            self.nc = 1
            self.decoder_dist = 'bernoulli'
            self.has_label = True
        elif args.dataset.lower()[:5] == 'cifar':
            self.nc = 3
            self.decoder_dist = 'gaussian'
            self.has_label = True
        else:
        ...
        if args.model == 'H':
            ...
            raise NotImplementedError('only support model H or B or mnist')

        self.net = cuda(net(z_dim=self.z_dim, nc=self.nc), self.use_cuda, self.gpu)
        print_size(self.net)
    ...
    def train(self):
        ...
        self.C_max = Variable(cuda(torch.FloatTensor([self.C_max]), self.use_cuda, self.gpu))
        out = False
        ...
        while not out:
            for x in self.data_loader:
                ...
                x = Variable(cuda(x, self.use_cuda, self.gpu))
    ...
    def viz_traverse(self, limit=3, inter=2/3, loc=-1):
        ...
        random_img = Variable(cuda(random_img, self.use_cuda, self.gpu), volatile=True).unsqueeze(0)
        random_img_z = encoder(random_img)[:, :self.z_dim]

        random_z = Variable(cuda(torch.rand(1, self.z_dim), self.use_cuda, self.gpu), volatile=True)
        ...
        if self.dataset == 'dsprites':
            ...
            fixed_img1 = Variable(cuda(fixed_img1, self.use_cuda, self.gpu), volatile=True).unsqueeze(0)
            fixed_img_z1 = encoder(fixed_img1)[:, :self.z_dim]

            fixed_img2 = self.data_loader.dataset.__getitem__(fixed_idx2)
            fixed_img2 = Variable(cuda(fixed_img2, self.use_cuda, self.gpu), volatile=True).unsqueeze(0)
            fixed_img_z2 = encoder(fixed_img2)[:, :self.z_dim]

            fixed_img3 = self.data_loader.dataset.__getitem__(fixed_idx3)
            fixed_img3 = Variable(cuda(fixed_img3, self.use_cuda, self.gpu), volatile=True).unsqueeze(0)
            ...
            fixed_img = Variable(cuda(fixed_img, self.use_cuda, self.gpu), volatile=True).unsqueeze(0)
